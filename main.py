...
def systemStatus():
    resp = requests.get(
        "https://kiki.ccu.edu.tw/~ccmisp06/cgi-bin/class_new/system_state.php")
    resp.encoding = 'utf8'
...
def announce():
    resp = requests.get(
        "https://kiki.ccu.edu.tw/~ccmisp06/cgi-bin/class_new/announce.php")
    resp.encoding = 'utf8'
    ...
    while True:
        ...
        for i in announcements:
            ...
            if(i["top"]):
                title += Fore.RED
            title += i["title"]
            print("{no} {date}: {title}".format(
                no=count, date=i["pub_date"], title=title))
        print()
        select = number_input(
            Fore.BLUE+"Select a announcement (enter 0 to exit):", False)
        if select <= 0:
            break
...
if __name__ == "__main__":
    ...
    print("Login as {id}: {name} {dept} {grade}{class_id}".format(
        id=login_status["id"], name=login_status["name"], dept=login_status["dept"], grade=login_status["grade"], class_id=login_status["class_id"]))

    sysStatus = systemStatus()
    print("\nSystem Status: {status_text}\nSemester: {semester}\n".format(
        status_text=sysStatus["status_text"], semester=sysStatus["semester"]))
    ...
    while True:
        ...
        if mode == 1:
            if(sysStatus["status_text"].find("暫不開放查詢") != -1):
                ...
                for i in selected:
                    print("{course_id}_{course_num}\t{name}\t{teacher}\t{time}\t{room}".format(course_id=i["code"], course_num=i["num"], name=i["name"].ljust(
                        20), teacher=i["teacher"].ljust(8), time=i["time"].ljust(20), room=i["room"]))
        elif mode == 2:
